...
\position{Doctorant Étudiant en Model-Driven Software Engineering} % Your expertise/fields
%\quote{``Make the change that you want to see in the world."} % A quote or statement

\makecvfooter{\today}{Van Cam. PHAM~~~·~~~Résumé}{\thepage} % Specify the letter footer with 3 arguments: (<left>, <center>, <right>), leave any of these blank if they are not needed
...
\input{cv-sections/education-fr.tex}

%\vspace{1cm}
%\input{cv-sections/employment}
\input{cv-sections/employment-fr}

%\newpage % Force a new page for looks
%\input{cv-sections/skills.tex}
\input{cv-sections/skiils-fr}
%\input{cv-sections/experience.tex}
%\vspace{1cm}
%\input{cv-sections/interests}
\input{cv-sections/interests-fr}
%\vspace{1cm}

%\input{cv-sections/references}
\input{cv-sections/references-fr}

%\newpage

%\input{cv-sections/publications}
\input{cv-sections/publications-fr}

%s\vspace{1cm}
%\input{cv-sections/presentation.tex}
\input{cv-sections/presentation-fr.tex}
